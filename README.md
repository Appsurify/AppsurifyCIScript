...
| teststorun | Defaults to "all" if runtemplate is not set.  Options include - high, medium, low, unassigned, ready, open, none.  Details below |
| startrunall | Required if testtemplate is not set and runtemplate is all tests or teststorun is set to all.  Command to start the test run to execute all tests.  Details below |
| endrunall | Command to end the test run to execute all tests.  Details below |
| startrunspecific | Required if testtemplate is not set and teststorun is not all or none or testtemplate is not all tests or no tests.  Command to start the test run to execute specific tests.  Details below  |
| endrunspecific | Command to end the test run to execute specific tests |
