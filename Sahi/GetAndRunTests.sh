...
json=""
if [[ $runfrequency == "single" && $repository == "git"]] ; then json=`curl --header "token: $apiKey" "$url/api/external/prioritized-tests/?project_name=$projectencoded&priority=$1&full_name=$fullname&test_suite_name=$testsuiteencoded&first_commit=$commitId&commit_type=Single" | sed 's/\"//g'` ; fi
if [[ $runfrequency == "multiple" && $repository == "git" ]] ; then json=`curl --header "token: $apiKey" "$url/api/external/prioritized-tests/?project_name=$projectencoded&priority=$1&full_name=$fullname&test_suite_name=$testsuiteencoded&first_commit=$commitId&commit_type=LastRun&target_branch=$branch" | sed 's/\"//g'` ; fi
if [[ $runfrequency == "betweenexclusive" && $repository == "git" ]] ; then json=`curl --header "token: $apiKey" "$url/api/external/prioritized-tests/?project_name=$projectencoded&priority=$1&full_name=$fullname&test_suite_name=$testsuiteencoded&first_commit=$commitId&commit_type=BetweenExclisuve&target_branch=$branch&second_commit=$fromcommit" | sed 's/\"//g'` ; fi
...
if [[ $runfrequency == "single" && $repository != "git"]] ; then json=`curl --header "token: $apiKey" "$url/api/external/prioritized-tests/?project_name=$projectencoded&priority=$1&full_name=$fullname&test_suite_name=$testsuiteencoded&first_commit=$commitId&commit_type=Single&repo=$repository" | sed 's/\"//g'` ; fi
if [[ $runfrequency == "multiple" && $repository != "git" ]] ; then json=`curl --header "token: $apiKey" "$url/api/external/prioritized-tests/?project_name=$projectencoded&priority=$1&full_name=$fullname&test_suite_name=$testsuiteencoded&first_commit=$commitId&commit_type=LastRun&target_branch=$branch&repo=$repository" | sed 's/\"//g'` ; fi
if [[ $runfrequency == "betweenexclusive" && $repository != "git" ]] ; then json=`curl --header "token: $apiKey" "$url/api/external/prioritized-tests/?project_name=$projectencoded&priority=$1&full_name=$fullname&test_suite_name=$testsuiteencoded&first_commit=$commitId&commit_type=BetweenExclisuve&target_branch=$branch&second_commit=$fromcommit&repo=$repository" | sed 's/\"//g'` ; fi
